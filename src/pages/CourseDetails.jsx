            ...
            <div className="lg:col-span-2 space-y-10">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {(course.features || [
                    ...
                    <motion.div
                      key={index}
                      whileHover={{ y: -2 }}
                      className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm flex items-start gap-3 hover:shadow-md transition-all duration-300"
                    >
                      <div className="mt-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 p-1 text-emerald-600 dark:text-emerald-400">
                        <Check className="w-3.5 h-3.5" />
                      </div>
                      <span className="text-sm font-medium text-slate-700 dark:text-slate-350">{feature}</span>
                    </motion.div>
                  ))}
                </div>
              </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      {Object.entries(course.level_structure).map(([level, data]) => (
                        <div key={level} className="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                {sortedLessons.length === 0 ? (
                  <p className="text-slate-500 dark:text-slate-400 text-center py-12 text-sm font-medium border border-slate-200/60 dark:border-slate-800 rounded-2xl">
                  ...
                  <Accordion type="single" collapsible className="space-y-3">
                    {sortedLessons.map((lesson, index) => (
                      <AccordionItem 
                        key={lesson.id} 
                        value={lesson.id}
                        className="border border-slate-200 dark:border-slate-800/80 rounded-2xl px-5 bg-white dark:bg-slate-900 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300"
                      >
                        <AccordionTrigger className="hover:no-underline py-4">
                          <div className="flex items-center gap-4 text-left">
                            <div className="w-9 h-9 rounded-xl bg-violet-50 dark:bg-violet-950/40 border border-violet-100/50 dark:border-violet-900/50 flex items-center justify-center text-violet-600 dark:text-violet-400 font-extrabold text-sm shrink-0">
                              {index + 1}
                            </div>
                            <div className="space-y-1">
                              ...
                              <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400 dark:text-slate-500 font-medium">
                                {lesson.type === 'video' && (
                                  <span className="flex items-center gap-1 bg-violet-500/10 text-violet-600 dark:text-violet-400 px-2 py-0.5 rounded-full text-[10px] font-bold">
                                    <Video className="w-3 h-3" /> Video
                                  </span>
                                )}
                                {lesson.type === 'live' && (
                                  <span className="flex items-center gap-1 bg-rose-500/10 text-rose-600 dark:text-rose-400 px-2 py-0.5 rounded-full text-[10px] font-bold">
                                    <CalendarDays className="w-3 h-3" /> Live
                                  </span>
                                )}
                                {lesson.type === 'reading' && (
                                  <span className="flex items-center gap-1 bg-amber-500/10 text-amber-600 dark:text-amber-400 px-2 py-0.5 rounded-full text-[10px] font-bold">
                                    <FileText className="w-3 h-3" /> Reading
                                  </span>
                                )}
                                <span className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full text-[10px] font-bold">
                                  <Clock className="w-3 h-3 text-slate-400" /> {lesson.duration_minutes || 10} min
                                </span>
                                {lesson.level && (
                                  <span className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full text-[10px] font-extrabold">
                                    <GraduationCap className="w-3 h-3 text-violet-500" /> {lesson.level}
                                  </span>
                                )}
                                {lesson.live_class_link && <span className="text-violet-600 dark:text-violet-400 font-semibold text-[10px]">• Live Link</span>}
                                {lesson.has_recording && <span className="text-emerald-600 dark:text-emerald-450 font-semibold text-[10px]">• Recording</span>}
                        ...
                        <AccordionContent className="pb-5 pt-2 border-t border-slate-100 dark:border-slate-800/60">
                          <div className="pl-13 space-y-4">
                            ...
                            {lesson.live_class_link && (
                              <div className="flex flex-wrap items-center gap-3 text-xs bg-slate-50 dark:bg-slate-800/40 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
                                <CalendarDays className="w-4.5 h-4.5 text-violet-500" />
                                <span className="text-slate-600 dark:text-slate-450 font-semibold">
                                  Class Scheduled: {lesson.live_class_date ? new Date(lesson.live_class_date).toLocaleString() : 'TBD'}
                                </span>
                                {enrollment && (
              ...
              {/* 3. Modernized Reviews Section */}
              <div className="space-y-6">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">
                  Student Feedback & Reviews
                </h3>

                <div className="grid md:grid-cols-3 gap-6 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                  {/* Average Rating Block */}
                  <div className="flex flex-col items-center justify-center text-center md:border-r border-slate-200 dark:border-slate-800 md:pr-6">
                    ...
                    <div className="flex items-center gap-0.5 my-3">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star 
                          key={star} 
                          className={`w-5 h-5 ${star <= Math.round(avgRating) ? 'text-amber-500 fill-amber-500' : 'text-slate-200 dark:text-slate-800'}`}
                        />
                      ))}
                    </div>
                    ...
                    {[5, 4, 3, 2, 1].map((rating) => {
                      ...
                      return (
                        <div key={rating} className="flex items-center gap-3">
                          <span className="text-xs font-bold text-slate-500 dark:text-slate-400 w-6 text-right">{rating}★</span>
                          <Progress value={percentage} className="flex-1 h-2 bg-slate-100 dark:bg-slate-800 [&>div]:bg-violet-600" />
                          <span className="text-xs font-bold text-slate-400 dark:text-slate-400 w-8 text-right">{count}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                {/* Reviews Grid */}
                {reviews.length === 0 ? (
                  <div className="text-center py-10 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-900/30">
                    <MessageCircle className="w-10 h-10 text-slate-300 dark:text-slate-600 mx-auto mb-3" />
                    <p className="text-slate-500 dark:text-slate-400 text-sm font-semibold">
                      No reviews for this course yet. Be the first to share your thoughts!
                    </p>
                  </div>
                ) : (
                  <div className="grid md:grid-cols-2 gap-4">
                    {reviews.map((review) => (
                      <div 
                        key={review.id} 
                        className="flex flex-col justify-between p-5 rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/40 shadow-sm hover:shadow-md transition-all duration-300"
                      >
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <Avatar className="w-9 h-9 border border-slate-100 dark:border-slate-800">
                                <AvatarFallback className="bg-violet-100 text-violet-700 font-extrabold text-xs">
                                  {review.user_name?.charAt(0).toUpperCase() || 'U'}
                                </AvatarFallback>
                              </Avatar>
                              <div>
                                <p className="font-bold text-slate-900 dark:text-white text-sm">
                                  {review.user_name || 'Verified Student'}
                                </p>
                                <span className="text-[9px] font-extrabold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-900/30 px-1.5 py-0.5 rounded uppercase tracking-wider">
                                  Verified Learner
                                </span>
                              </div>
                            </div>
                            <div className="flex items-center gap-0.5">
                              {[...Array(5)].map((_, i) => (
                                <Star 
                                  key={i} 
                                  className={`w-3.5 h-3.5 ${i < review.rating ? 'text-amber-500 fill-amber-500' : 'text-slate-200 dark:text-slate-800'}`} 
                                />
                              ))}
                            </div>
                          </div>
              ...
              <div className="space-y-4 pt-4">
                ...
                <Accordion type="single" collapsible className="space-y-2">
                  <AccordionItem value="faq-1" className="border border-slate-200 dark:border-slate-800/80 rounded-2xl px-5 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
                    <AccordionTrigger className="hover:no-underline font-semibold text-slate-850 dark:text-white text-sm py-4">
                    ...
                    <AccordionContent className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed pb-4">
                      Yes! The A1 level courses are designed precisely for absolute beginners. We build your knowledge from alphabet and pronunciation rules up to conversational blocks.
                    </AccordionContent>
                  </AccordionItem>
                  <AccordionItem value="faq-2" className="border border-slate-200 dark:border-slate-800/80 rounded-2xl px-5 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
                    <AccordionTrigger className="hover:no-underline font-semibold text-slate-850 dark:text-white text-sm py-4">
                      How do the live sessions work?
                    </AccordionTrigger>
                    <AccordionContent className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed pb-4">
                      Live classes are held via Google Meet with native speakers. The links are accessible right from your learning dashboard. Recordings are uploaded within 24 hours of completion.
                    </AccordionContent>
                  </AccordionItem>
                  <AccordionItem value="faq-3" className="border border-slate-200 dark:border-slate-800/80 rounded-2xl px-5 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
                    <AccordionTrigger className="hover:no-underline font-semibold text-slate-850 dark:text-white text-sm py-4">
                      What is the refund policy?
                    </AccordionTrigger>
                    <AccordionContent className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed pb-4">
                      We offer a 30-day money-back guarantee. If you are not satisfied with the learning materials or instructor quality, you can submit a refund claim within 30 days of purchasing.
                    </AccordionContent>
                  </AccordionItem>
                  <AccordionItem value="faq-4" className="border border-slate-200 dark:border-slate-800/80 rounded-2xl px-5 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
                    <AccordionTrigger className="hover:no-underline font-semibold text-slate-850 dark:text-white text-sm py-4">
                      Will I get a certificate of completion?
                    </AccordionTrigger>
                    <AccordionContent className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed pb-4">
                      Yes! Upon complete attendance and completion of curriculum lessons, assignments, and test quizzes, a certificate will be made available in your dashboard.
                    </AccordionContent>
                  </AccordionItem>
                </Accordion>
              </div>
              ...
              <Card className="border border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-slate-900 sticky top-24 rounded-2xl overflow-hidden">
                <CardContent className="p-6 space-y-6">
                  ...
                  {enrollment ? (
                    <div className="space-y-3">
                      <Link to={createPageUrl(`CoursePlayer?enrollmentId=${enrollment.id}`)}>
                        <Button className="w-full bg-violet-600 hover:bg-violet-700 text-white h-12 text-sm font-bold shadow-md shadow-violet-600/25 hover:shadow-lg rounded-xl">
                    ...
                    <div className="space-y-3">
                      ...
                      {isInCart ? (
                        <Button
                          className="w-full bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-300 dark:border-emerald-900/35 rounded-xl h-12 font-bold flex items-center justify-center gap-2 transition-all"
                        ...
                        <Button
                          variant="outline"
                          className="w-full border-slate-200 dark:border-slate-800 text-slate-850 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-900 rounded-xl h-12 font-bold flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
                      ...
                      {/* Buy Now button */}
                      <Button 
                        className="w-full bg-violet-600 hover:bg-violet-700 text-white h-12 text-sm font-bold shadow-md shadow-violet-600/25 hover:shadow-lg rounded-xl transition-all"
                        onClick={handleEnroll}
                        disabled={enrollMutation.isPending}
                      >
                        {enrollMutation.isPending ? 'Enrolling...' : 'Buy Now'}
                      </Button>
                    </div>
                  )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3.5 text-slate-600 dark:text-slate-400 text-xs font-semibold">
          ...
          <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-slate-900/95 border-t border-slate-200 dark:border-slate-800 p-4 shadow-[0_-8px_30px_rgb(0,0,0,0.08)] backdrop-blur-md transition-all duration-300">
            <div className="max-w-md mx-auto flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 flex-1 justify-end">
                ...
                <Button
                  size="sm"
                  className="bg-violet-600 hover:bg-violet-700 text-white rounded-xl h-11 px-5 font-bold flex-1 max-w-[160px]"
                  onClick={handleEnroll}
                  disabled={enrollMutation.isPending}
                >
                  {enrollMutation.isPending ? '...' : 'Buy Now'}
                </Button>
              </div>
            </div>
          </div>
